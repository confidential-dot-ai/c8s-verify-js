...
function nameCN(buf, nameNode) {
  for (const rdn of readChildren(buf, nameNode)) {
    ...
    for (const atv of readChildren(buf, rdn)) {
      ...
      if (parts.length === 2 && parts[0].tag === TAG.OID && decodeOID(parts[0].content) === OID.CN) {
...
export function parseCertificate(der) {
  ...
  if (tbsChildren[i] && tbsChildren[i].tag === 0xa0) i++;
  ...
  let spkiCurve = null;
  if (spkiChildren[0] && spkiChildren[0].tag === TAG.SEQUENCE) {
    const algParts = readChildren(der, spkiChildren[0]);
    if (algParts[1] && algParts[1].tag === TAG.OID) {
 ...
 * @param {Uint8Array} der
 * @param {number} size curve component size in bytes
 * @returns {Uint8Array}
 */
function ecdsaDerToRaw(der, size) {
  ...
  /** @param {Uint8Array} int @param {number} off */
  const place = (int, off) => {
    let v = int;
 ...
 * @param {Certificate|Uint8Array} cert
...
export async function verifySignedBy(child, issuer, opts = {}) {
  ...
  for (const [label, c] of [["leaf", child], ["CA", issuer]]) {
    if (at < c.notBefore) {
 ...
 * @param {Uint8Array} leafDer
 * @param {Uint8Array} caDer
 * @param {{ at?: Date }} [opts]
 * @returns {Promise<{ leaf: Certificate, ca: Certificate, leafSha256: string, caSha256: string }>}
 */
